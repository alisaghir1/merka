...
export default function Services() {
  ...
  useEffect(() => {
    ...
    const fetchServices = async () => {
      try {
        const data = await getServices({ published: true })
        if (data && data.length > 0) {
          setServices(data)
        }
      } catch (error) {
        console.log('Using static services data:', error)
      }
    }
    fetchServices()
            ...
            {services.map((service, index) => (
              ...
              >
                ...
                <div className="relative bg-white p-8 rounded-3xl shadow-lg hover:shadow-2xl hover:-translate-y-4 transition-all duration-500 border border-gray-100 group-hover:border-[#877051]/30 overflow-hidden">
                  {/* Background number */}
                  <div className="absolute -top-4 -right-4 text-9xl font-bold text-gray-100 group-hover:text-[#877051]/10 transition-colors duration-500 select-none">
                    {String(index + 1).padStart(2, '0')}
                  </div>
                  
                  <div className="relative z-10">
                    {/* Service icon */}
                    <div className={`w-16 h-16 bg-gradient-to-r ${service.gradient} rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 shadow-lg`}>
                  ...
                  <Link href={`/services/${service.slug}`}>
                    <div className="flex items-center text-[#877051] font-semibold group-hover:text-[#041533] transition-colors duration-300">
                      <span className="group-hover:translate-x-1 transition-transform duration-300">Explore Service</span>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <div 
                key={index}
                className={`text-center group p-8 bg-white rounded-3xl border border-gray-100 hover:border-[#877051]/30 hover:shadow-2xl hover:-translate-y-3 transition-all duration-500 ${
                  scrollValue > 2400 + index * 200 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                }`}
                style={{ transitionDelay: `${index * 200}ms` }}
              >
                <div className="w-20 h-20 bg-gradient-to-r from-[#041533] to-[#877051] rounded-3xl flex items-center justify-center mx-auto mb-6 group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 shadow-xl">
