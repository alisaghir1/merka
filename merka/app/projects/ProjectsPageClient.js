...
export default function ProjectsPageClient({ initialProjects = [], initialCategories = [] }) {
      ...
      <section className="py-20 bg-white relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          >
            ...
            <p className={`text-xl text-gray-600 max-w-3xl mx-auto ${isRTL ? 'text-right' : ''}`}>
          ...
          >
            {[{ id: 'all', name: t('projects.filter.all'), count: projects.length }, ...categories
              .filter(cat => (cat.id || cat) !== 'all')
              .map(cat => {
                const catId = cat.id || cat
                const catKey = catId.toLowerCase().replace('-', '')
                const translatedName = t(`projects.filter.${catKey}`) || t(`projects.filter.${catId}`) || cat.name || catId.charAt(0).toUpperCase() + catId.slice(1)
                return {
                  id: catId,
                  name: translatedName,
            ...
            {filteredProjects.map((project, index) => (
              ...
              >
                ...
                <div className="relative h-64 overflow-hidden">
                  ...
                  >
                    <div className={`text-center text-white p-6 ${isRTL ? 'text-right' : ''}`}>
                      <h4 className="text-lg font-bold mb-2">{language === 'ar' && project.title_ar ? project.title_ar : project.title}</h4>
                      <p className="text-sm opacity-90 mb-4">{language === 'ar' && project.description_ar ? project.description_ar : project.description}</p>
                ...
                <div className={`p-6 ${isRTL ? 'text-right' : ''}`}>
                  <div className={`flex items-start justify-between mb-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    ...
                    <span className={`text-sm text-gray-500 ${isRTL ? 'mr-2' : 'ml-2'}`}>{project.year}</span>
                  </div>
                  
                  <div className="space-y-2 mb-4">
                    <div className={`flex items-center text-sm text-gray-600 ${isRTL ? 'flex-row-reverse' : ''}`}>
                      <span className={isRTL ? 'ml-2' : 'mr-2'}>📍</span>
                      {language === 'ar' && project.location_ar ? project.location_ar : project.location}
                    </div>
                    <div className={`flex items-center text-sm text-gray-600 ${isRTL ? 'flex-row-reverse' : ''}`}>
                      <span className={isRTL ? 'ml-2' : 'mr-2'}>📐</span>
                      {language === 'ar' && project.area_ar ? project.area_ar : project.area}
                    </div>
                  </div>
                  ...
                  <div className={`flex flex-wrap gap-2 mb-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    ...
                    {project.services?.length > 2 && (
                      <span className="px-2 py-1 bg-[#041533]/10 text-[#041533] rounded-lg text-xs">
                        +{project.services.length - 2} {t('projects.moreServices')}
                      </span>
                    )}
                  </div>
                  ...
                  <Link href={`/projects/${project.slug}`}>
                    <button className="w-full bg-gradient-to-r from-[#041533] to-[#877051] text-white py-3 rounded-lg font-semibold hover:shadow-lg hover:scale-105 transition-all duration-300 text-center">
                      {t('projects.viewProjectDetails')}
                    </button>
                  </Link>
                </div>
              </div>
      ...
      <section className="py-20 bg-gray-50 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className={`grid md:grid-cols-4 gap-8 ${isRTL ? 'direction-rtl' : ''}`}>
            {[
              { number: '150+', label: t('projects.stats.projectsCompleted'), icon: '🏗️' },
              { number: '2.5M+', label: t('projects.stats.squareFeet'), icon: '📐' },
              { number: '95%', label: t('projects.stats.clientSatisfaction'), icon: '⭐' },
              { number: '12+', label: t('projects.stats.yearsExperience'), icon: '🏆' }
            ].map((stat, index) => (
              <div 
                key={index} 
        ...
        <div className="max-w-4xl mx-auto text-center px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          >
            ...
            <div className={`flex flex-col sm:flex-row gap-6 justify-center ${isRTL ? 'sm:flex-row-reverse' : ''}`}>
              <Link href="/contact">
                <button className={`group bg-white text-[#041533] px-10 py-5 rounded-xl font-semibold text-lg hover:shadow-2xl hover:scale-105 transition-all duration-500 relative overflow-hidden ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <span className="absolute inset-0 bg-gradient-to-r from-[#877051] to-[#041533] opacity-0 group-hover:opacity-100 transition-opacity duration-500"></span>
                  <span className="relative z-10 group-hover:text-white transition-colors duration-500">
              ...
              <Link href="/services">
                <button className={`group border-2 border-white text-white px-10 py-5 rounded-xl font-semibold text-lg hover:shadow-2xl hover:scale-105 transition-all duration-500 relative overflow-hidden ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <span className="absolute inset-0 bg-white opacity-0 group-hover:opacity-100 transition-opacity duration-500"></span>
                  <span className="relative z-10 group-hover:text-[#041533] transition-colors duration-500">
                    {t('projects.cta.ourServices')}
                  </span>
                </button>
              </Link>
            </div>
