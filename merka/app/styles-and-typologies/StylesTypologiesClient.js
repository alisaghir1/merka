...
export default function StylesTypologiesClient({ initialStyles = [], initialTypologies = [] }) {
  const [mounted, setMounted] = useState(false)
  const [scrollY, setScrollY] = useState(0)
  const [isLoaded, setIsLoaded] = useState(false)
  // Use server-fetched data or fallback to static
  const styles = initialStyles.length > 0 ? initialStyles : staticStylesData
  const typologies = initialTypologies.length > 0 ? initialTypologies : staticTypologiesData
    ...
    <div className="min-h-screen bg-white overflow-x-hidden">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className={`transition-all duration-1000 ${isLoaded ? 'opacity-0 scale-50' : 'opacity-100 scale-100'}`}>
              ...
              <h1 className="text-4xl md:text-6xl font-serif font-bold text-white text-center">
                {t('stylesTypologies.hero.loading')}
              </h1>
      ...
      >
        ...
        >
          ...
          <h1 className="text-6xl md:text-8xl lg:text-9xl font-serif font-bold text-white mb-6 leading-tight">
            ...
            >
              {t('stylesTypologies.hero.titleAnd')}
            </span>
            <span 
            ...
            >
              {t('stylesTypologies.hero.titleSecond')}
            </span>
          </h1>
          ...
          >
            {t('stylesTypologies.hero.subtitle')}
          </p>
        </div>
        ...
        >
          <div className="flex flex-col items-center animate-bounce">
            <span className="text-sm mb-2">{t('stylesTypologies.scrollDown')}</span>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          >
            <h2 className="text-4xl md:text-5xl font-serif font-bold text-[#041533] mb-6">
              {t('stylesTypologies.styles.title')}
            </h2>
            ...
            <p className="text-xl text-gray-600 max-w-4xl mx-auto mt-6">
              {t('stylesTypologies.styles.subtitle')}
            </p>
          </div>
              ...
              <Link key={style.id || index} href={`/styles-and-typologies/styles/${style.slug}`} className="block">
                ...
                >
                  ...
                  <div className="relative bg-white p-8 rounded-3xl shadow-lg hover:shadow-2xl hover:-translate-y-4 transition-all duration-500 border border-gray-100 group-hover:border-[#877051]/30 overflow-hidden">
                    {/* Background Number */}
                    <div className={`absolute ${isRTL ? '-left-4' : '-right-4'} -top-4 text-[8rem] font-black text-gray-100/50 pointer-events-none select-none group-hover:text-[#877051]/10 transition-colors duration-500`}>
                      {String(index + 1).padStart(2, '0')}
                    </div>
                    
                    <div className={`relative w-20 h-20 bg-gradient-to-r ${style.gradient} rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 shadow-lg ${isRTL ? 'mr-0 ml-auto' : ''}`}>
                    ...
                    <h3 className="text-2xl font-serif font-bold text-[#041533] mb-4 group-hover:text-[#877051] transition-colors duration-300">
                      {language === 'ar' && style.title_ar ? style.title_ar : style.title}
                    </h3>
                    ...
                    <div className={`flex items-center text-[#877051] font-semibold group-hover:text-[#041533] transition-colors duration-300 ${isRTL ? 'flex-row-reverse' : ''}`}>
                      <span className={`transition-transform duration-300 ${isRTL ? 'group-hover:-translate-x-1' : 'group-hover:translate-x-1'}`}>{t('stylesTypologies.styles.exploreStyle')}</span>
      ...
      <section className="py-20 bg-gray-50 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          >
            <h2 className="text-4xl md:text-5xl font-serif font-bold text-[#041533] mb-6">
              {t('stylesTypologies.typologies.title')}
            </h2>
            <div className="w-24 h-1 bg-gradient-to-r from-[#877051] to-[#041533] rounded-full mx-auto"></div>
            <p className="text-xl text-gray-600 max-w-4xl mx-auto mt-6">
              {t('stylesTypologies.typologies.subtitle')}
            </p>
          </div>

          <div className={`grid lg:grid-cols-2 gap-8 ${isRTL ? 'text-right' : ''}`}>
              ...
              <Link key={typology.id || index} href={`/styles-and-typologies/typologies/${typology.slug}`} className="block">
                ...
                >
                  ...
                  <div className="relative bg-white p-8 rounded-3xl shadow-lg hover:shadow-2xl hover:-translate-y-4 transition-all duration-500 border border-gray-100 group-hover:border-[#877051]/30 overflow-hidden">
                    ...
                    <div className={`relative w-20 h-20 bg-gradient-to-r ${typology.gradient} rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 shadow-lg ${isRTL ? 'mr-0 ml-auto' : ''}`}>
                      <span className="text-white text-3xl">{typology.icon}</span>
                    </div>
                    
                    <h3 className="text-2xl font-serif font-bold text-[#041533] mb-4 group-hover:text-[#877051] transition-colors duration-300">
                      {language === 'ar' && typology.title_ar ? typology.title_ar : typology.title}
                    </h3>
                    ...
                    <div className={`flex items-center text-[#877051] font-semibold group-hover:text-[#041533] transition-colors duration-300 ${isRTL ? 'flex-row-reverse' : ''}`}>
                      <span className={`transition-transform duration-300 ${isRTL ? 'group-hover:-translate-x-1' : 'group-hover:translate-x-1'}`}>{t('stylesTypologies.typologies.viewTypology')}</span>
                      <svg className={`w-5 h-5 transition-transform duration-300 ${isRTL ? 'mr-2 rotate-180 group-hover:-translate-x-2' : 'ml-2 group-hover:translate-x-2'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                      </svg>
                    </div>
      ...
      <section className="py-20 bg-white relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h2 className="text-4xl md:text-5xl font-serif font-bold text-[#041533] mb-6">
              {t('stylesTypologies.whyApproach.title')}
            </h2>
            <div className="w-24 h-1 bg-gradient-to-r from-[#877051] to-[#041533] rounded-full mx-auto"></div>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {t('stylesTypologies.whyApproach.benefits').map((benefit, index) => (
              <div 
                key={index} 
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-white text-center">
          ...
          >
            <h2 className="text-4xl md:text-5xl font-serif font-bold mb-6">
              {t('stylesTypologies.cta.title')}
            </h2>
            <p className="text-xl text-gray-200 mb-8">
              {t('stylesTypologies.cta.description')}
            </p>
            
            <div className={`flex flex-col sm:flex-row gap-6 justify-center ${isRTL ? 'sm:flex-row-reverse' : ''}`}>
              <Link href="/contact">
                <button className={`group bg-white text-[#041533] px-10 py-5 rounded-2xl font-semibold shadow-xl hover:shadow-2xl hover:scale-105 transition-all duration-500 relative overflow-hidden flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  ...
                  <svg className={`relative z-10 w-5 h-5 group-hover:text-white transition-all duration-300 ${isRTL ? 'rotate-180 group-hover:-translate-x-1' : 'group-hover:translate-x-1'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                </button>
              </Link>
              <Link href="/projects">
                <button className={`group border-2 border-white text-white px-10 py-5 rounded-2xl font-semibold backdrop-blur-sm hover:shadow-2xl hover:scale-105 transition-all duration-500 relative overflow-hidden flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  ...
                  <span className="relative z-10 group-hover:text-[#041533] transition-colors duration-500">
                    {t('stylesTypologies.cta.viewPortfolio')}
                  </span>
                  <svg className={`relative z-10 w-5 h-5 group-hover:text-[#041533] transition-all duration-300 ${isRTL ? 'rotate-180 group-hover:-translate-x-1' : 'group-hover:translate-x-1'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                </button>
              </Link>
            </div>
          </div>
