...
export default function HomeClient({ initialServices = [], initialFeaturedProjects = [] }) {
  ...
  // Use server-fetched data or fallback
    ...
    <div className="min-h-screen overflow-x-hidden">
          ...
          >
            <div className={`bg-white/95 backdrop-blur-md p-8 rounded-3xl shadow-2xl border border-white/50 ${isRTL ? 'text-right' : ''}`}>
              <h3 className="text-3xl font-serif font-bold text-[#041533] mb-4">
                {t('home.inspires.title')}
              </h3>
              <p className="text-lg text-gray-700 mb-6 leading-relaxed">
                {t('home.inspires.description')}
              </p>
              <div className={`flex flex-col sm:flex-row gap-4 justify-center ${isRTL ? 'sm:flex-row-reverse' : ''}`}>
                <Link href="/services">
                  <button className={`bg-[#041533] text-white px-6 py-3 rounded-lg font-medium hover:bg-[#877051] hover:scale-105 transition-all duration-300 flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    {t('common.discoverProcess')}
      ...
      <section className="py-20 bg-white relative z-30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className={`grid md:grid-cols-3 gap-8 ${isRTL ? 'text-right' : ''}`}>
            {[
              { icon: "🏗️", title: t('home.philosophy.innovation'), desc: t('home.philosophy.innovationDesc') },
              { icon: "🌿", title: t('home.philosophy.sustainability'), desc: t('home.philosophy.sustainabilityDesc') },
              { icon: "🎨", title: t('home.philosophy.culturalFusion'), desc: t('home.philosophy.culturalFusionDesc') }
            ].map((item, index) => (
              <div 
                key={index}
      ...
      <section className="relative h-screen overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none">
          ...
          >
            <div className={`bg-black/30 backdrop-blur-md p-8 rounded-3xl border-2 border-white/20 shadow-2xl ${isRTL ? 'text-right' : ''}`}>
              ...
              <Link href="/projects">
                <button className={`bg-white text-[#041533] px-8 py-4 rounded-lg font-semibold text-lg hover:bg-gray-50 hover:scale-105 transition-all duration-300 flex items-center gap-2 mx-auto ${isRTL ? 'flex-row-reverse' : ''}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.slice(0, 6).map((service, index) => (
              ...
              >
                {/* Background number */}
                <div className={`absolute -top-4 ${isRTL ? '-left-4' : '-right-4'} text-8xl font-bold text-gray-100 group-hover:text-[#877051]/10 transition-colors duration-500 select-none`}>
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-gray-800 group-hover:text-gray-900 transition-colors duration-300 mb-6 line-clamp-2">
                    {isRTL && service.description_ar ? service.description_ar : (service.short_description || service.description)}
                  </p>
                  
                  <div className={`flex items-center text-[#877051] font-semibold ${isRTL ? 'flex-row-reverse' : ''}`}>
                    <span className={`text-sm ${isRTL ? 'ml-2' : 'mr-2'}`}>{t('common.learnMore')}</span>
                    <svg className={`w-5 h-5 transition-transform ${isRTL ? 'group-hover:-translate-x-2 rotate-180' : 'group-hover:translate-x-2'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                    </svg>
                  </div>
                </div>
          ...
          >
            <Link href="/services">
              <button className={`group bg-gradient-to-r from-[#041533] to-[#877051] text-white px-10 py-5 rounded-2xl font-semibold text-lg hover:shadow-2xl hover:scale-105 transition-all duration-500 flex items-center gap-3 mx-auto ${isRTL ? 'flex-row-reverse' : ''}`}>
                {t('common.viewAll')}
                <svg className={`w-5 h-5 transition-transform ${isRTL ? 'group-hover:-translate-x-2 rotate-180' : 'group-hover:translate-x-2'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                </svg>
                ...
                >
                  ...
                  <div className="p-6 relative">
                    ...
                    <p className="text-gray-800 text-sm mb-4 line-clamp-2 leading-relaxed">
                      {isRTL && project.description_ar ? project.description_ar : (project.short_description || project.description)}
                    </p>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      <div className="flex items-center text-gray-500">
                        ...
                        <span>{isRTL && project.location_ar ? project.location_ar : (project.location || 'Dubai, UAE')}</span>
                      </div>
                      
                      <div className={`flex items-center text-[#877051] font-semibold group-hover:translate-x-1 transition-transform duration-300 ${isRTL ? 'flex-row-reverse group-hover:-translate-x-1' : ''}`}>
                        <span className={isRTL ? 'ml-1' : 'mr-1'}>{t('home.featuredProjects.viewButton')}</span>
              ...
              <Link href="/projects">
                <button className="group relative overflow-hidden bg-gradient-to-r from-[#041533] to-[#877051] text-white px-12 py-5 rounded-2xl font-semibold text-lg shadow-xl hover:shadow-2xl transition-all duration-500 hover:scale-105">
                  ...
                  <span className={`relative z-10 flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    {t('home.featuredProjects.exploreAll')}
                    <svg className={`w-5 h-5 transition-transform duration-300 ${isRTL ? 'rotate-180 group-hover:-translate-x-2' : 'group-hover:translate-x-2'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                    </svg>
                  </span>
                </button>
        ...
        <div className="max-w-4xl mx-auto text-center px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <div className={`flex flex-col sm:flex-row gap-6 justify-center px-4 ${isRTL ? 'sm:flex-row-reverse' : ''}`}>
              <Link href="/contact">
                <button className={`group bg-white text-[#041533] px-8 py-4 rounded-2xl font-semibold text-lg hover:bg-[#041533] hover:text-white hover:scale-105 transition-all duration-500 shadow-xl hover:shadow-2xl flex items-center gap-2 justify-center ${isRTL ? 'flex-row-reverse' : ''}`}>
                  {t('home.cta.button')}
                  <svg className={`w-5 h-5 transition-transform ${isRTL ? 'group-hover:-translate-x-1 rotate-180' : 'group-hover:translate-x-1'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                  </svg>
